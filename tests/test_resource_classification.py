...
class ResourceClassificationTests(unittest.TestCase):
    def test_classifies_compose_project_and_recommends_dockerizer(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(payload["root_classification"]["platform_delivery"]["blocking_until_confirmed"])
            self.assertEqual(payload["root_classification"]["platform_delivery"]["auto_dockerizer"], "must_use_dockerizer")
    ...
    def test_cli_writes_handoff_tables_for_dockerizer_route(self):
        ...
        self.assertEqual(docker_rows[0]["自动动作"], "auto_action=auto-render")
