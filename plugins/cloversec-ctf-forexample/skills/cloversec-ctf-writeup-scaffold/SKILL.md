...
description: 四叶草安全-创研中心竞赛专用题目手册构建 Skills，面向题目源码、WP、构建产物与零散人工材料的标准化整理：自动提取考点、部署信息、完整 Flag、HUB录题字段，并生成符合内部样例的正式中文题目手册；hub_fields.json 和 xlsx_fields.json 必须保留完整 Flag。
---
...
把题目源码、WP、构建产物和人工材料整理成内部题目手册与录题字段。默认产物：
...
python3 scripts/validate_manual.py <输出目录>/题目解题手册.md
```
...
- `分类-题目名.md` 是对人交付的正式手册文件，命名参考 `WEB-EZsmuggler.md`。
- `题目解题手册.md` 是兼容读取别名，内容与正式手册一致。
