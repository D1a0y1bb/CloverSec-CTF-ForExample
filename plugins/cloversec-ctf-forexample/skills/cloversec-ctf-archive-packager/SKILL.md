...
- 机器 manifest 写入 `_cache/`，不进入对人交付目录。
...
批量模式会为每道题生成归档目录，写入 `_cache/batch_archive_summary.json`、`_cache/batch_archive_report.md`，并可生成最终 xlsx 和语雀表。镜像 tar 默认只记录原路径和 hash，不复制进归档目录；确实需要复制时显式加 `--copy-image-tars`。
...
- 手册和题目名称一致。
- `_cache/archive_manifest.json` 的 `issues` 为空时，才把 `是否归档` 建议为 `是`。
