...
批量模式会为每道题生成归档目录，写入 `_batch/batch_archive_summary.json`、`_batch/batch_archive_report.md`，并可生成最终 xlsx 和语雀表。镜像 tar 默认只记录原路径和 hash，不复制进归档目录；确实需要复制时显式加 `--copy-image-tars`。
