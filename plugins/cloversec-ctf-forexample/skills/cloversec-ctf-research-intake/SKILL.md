...
2. 先跑免费源：CTFTime events/writeups、GitHub repository search、公开归档 seeds、CTF 平台 seeds、DuckDuckGo HTML、CSDN/博客园/语雀 `site:` 定向搜索。
3. 如果用户提供 key，再启用 GitHub code search、Brave Search、Bing Search。
4. 每条记录保留来源链接、访问日期、证据摘要。
5. 对缺少来源、题名、分类、附件线索的记录标记 `无法确认`。
6. 输出结构化结果，不把推断内容写成事实。
...
没有 key 时仍使用 GitHub repository search、CTFTime、DuckDuckGo HTML、内置公开归档 seeds、CTF 平台 seeds，以及 CSDN/博客园/语雀 `site:` 定向搜索。
...
- `seeds`：公开归档仓库 seeds。
- `ctf-platforms`：CTFTime、NSSCTF、CTFHub、攻防世界、BUUOJ、picoCTF、pwn.college、Root-Me 等平台 seeds。
