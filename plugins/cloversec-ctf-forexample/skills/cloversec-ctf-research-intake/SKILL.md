...
用户说“收集一道 CTF 题目”“帮我找一道题”“完整收集一道题”时，默认目标是公开真实比赛题目：先找赛事、题名、官方题包、源码、附件、WP 和证据。除非用户明确说“出题”“新建题目”“原创题”，不要自己写一道新题来代替收集。
...
16. `ctf_cases.jsonl` 不能为空。已有 `search_results.json` 但题目清单为空时，重新从结果生成 case；还是没有 case 时继续搜索，不要把空清单交给后续阶段。
17. 用户说“完整收集一道题”时，research 只是第一步；拿到真实题目后要交给 workflow 继续做材料整理、Dockerizer/附件分流、手册、质量检查和中文最终交付。
