...
```bash
python3 plugins/cloversec-ctf-forexample/scripts/cloversec_ctf_package.py inspect <附件.zip> --output attachment_manifest.json
```

生成多个附件包 manifest：

```bash
python3 plugins/cloversec-ctf-forexample/scripts/cloversec_ctf_package.py manifest <附件1.zip> <附件2.zip> --output attachment_manifest.json
```

manifest 会记录压缩包类型、大小、SHA256、目录项、是否可解压和路径穿越风险。
