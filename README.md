...
<p align="center">
  <a href="https://github.com/D1a0y1bb/CloverSec-CTF-ForExample/releases"><img alt="Version" src="https://img.shields.io/badge/version-v0.5.3-2563eb"></a>
...
| 手册撰写 | 中文正式手册、Hub 字段、xlsx 字段、完整 Flag 字段 |
...
| 最终交付 | `交付说明.md`、`最终表格/最终归档表.xlsx`、`语雀归档表/语雀粘贴表.md`、`手册/题目解题手册.md`、`质量检查报告/最终报告.md` |
...
| 题目收集 | `开始收集候选题和证据，不下载正式附件` | `search_results.json`、`ctf_cases.jsonl`、`evidence/`、`当前状态.md` |
| 材料整理 | `根据 ctf_cases.jsonl 下载源码、附件和 WP 到沙箱` | `downloads_sandbox/`、`material_candidates.json` |
...
| 手册字段 | `生成手册、Hub 字段和 xlsx 字段，完整 Flag 写入内部字段` | `题目解题手册.md`、`hub_fields.json`、`xlsx_fields.json` |
...
| 过程兼容文件 | `保留脚本和批处理需要的英文副本` | `过程证据/机器数据/` |
...
`当前状态.md` 是给人看的状态文件，只写处理了几题、缺材料几题、待确认几题和最终文件位置。日常接手优先看它，不需要翻一堆过程 JSON。
...
- 只有 `challenge.yml`、solver、writeup、截图、Flag 或平台页面时，只能算线索或手册材料，不能写成可交付题目。
...
`manual_filled_draft.md` 还会保留给旧流程兼容，但正式交付和 Hub 默认使用 `题目解题手册.md`。

### 检查手册质量

```text
检查这个 题目解题手册.md、ctf_case.json 和 hub_fields.json 是否能进入 Hub 草稿。
```
...
脚本仍会同时写出 `final_report.md`、`archive.xlsx`、`yuque_table.md`、`final_report.json`，这些是兼容旧批处理和已有测试数据的过程文件，不作为给人接手看的最终交付名称。中文交付包会把 JSON 和英文兼容文件放到 `过程证据/机器数据/`。
...
- `v0.5.3` 会优先识别 `challenge.yml`、GitHub 题目目录和 metadata 里的附件路径，尽量直接生成具体题目的 `ctf_cases.jsonl`，而不是把 CTFtime 汇总页当成题目本体。Medium、InfosecWriteups 等返回 403 时，可以把可见页面标题、链接、摘要或正文片段交给浏览器辅助导入工具。
